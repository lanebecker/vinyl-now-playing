      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* eyebrow — only shown in compact variant (where the top
              ...
              fontSize: 12 / letterSpacing: 0.22em intentionally exceeds the
              Label spec (11px / 0.16em): this line carries more info density
              than a typical label — status + side/position counter on a single
              line — and benefits from the extra breathing room. */}
          {variant === 'compact' && (
            <div style={{
          ...
          {/* NEXT eyebrow — between state only; separates the "what's coming"
              label from the track name so the hero can be the full 72px. */}
          {state === 'between' && (
            <div style={{
              fontFamily: '"JetBrains Mono", monospace',
          ...
          <div style={{
            ...
            textWrap: 'balance',
            marginBottom: 22,
            paddingBottom: '0.12em', // prevents descender clipping from overflow:hidden + tight line-height; CSS-only fix, no production equivalent
            overflow: 'hidden',
            display: '-webkit-box',
            WebkitLineClamp: 3,
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 'auto', minHeight: 28 }}>
            {(album.genres ?? []).slice(0, 3).map(g => (
              <span key={g} style={{
                ...
                color: p.muted, border: `1px solid ${p.accent}55`,
                borderRadius: 2,
              }}>{g}</span>
            ))}
  ...
  if (state === 'error') {
    return (
      <div style={{
        ...
        {/* Recovery hint — gives the room listener an action to try */}
        <div style={{
          fontFamily: '"JetBrains Mono", monospace',
          fontSize: 11, letterSpacing: '0.12em', color: p.muted,
          marginTop: -10,
        }}>REPOSITION NEEDLE TO RETRY</div>
      </div>
    );
  }
  if (state === 'boot') {
...
function trackText(album, state) {
  ...
  if (state === 'between') return album.next?.track ?? '—';
  return album.track;
}
